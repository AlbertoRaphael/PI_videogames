...
const infoApi = async () => {
    ...
    try {
        for (let i = 1; i <= 5; i++) { //con un for recorro mi API, ya que es un arreglo, 5 veces
            let api = await axios.get(url) //realizo la peticion
            //en mi .data podemos encontrar dos propiedades, results que es es aquello que voy a mapear
            api.data.results.map(v => { //a la respuesta/resultado lo mapeo
                videojuegos.push({ //y pusheo en mi array vacio todo aquello que mapee
                    id: v.id,
                    name: v.name,
                    image: v.background_image|| "https://cdn.drawception.com/drawings/fhrR1T1nq6.png",
                    //released: e.released,
                     rating: v.rating,
...
const infoTotal = async () => {
    ...
    const infoCompleta = [].concat(dbData).concat(apiData)
   // console.log(apiData);
    return infoCompleta
}
//infoApi()
